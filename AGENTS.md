# Guía para Agentes de IA - Geo-Rect (Validación Geoespacial de Manzanas)

Este archivo adapta el formato `AGENTS.md` al proyecto **Geo-Rect** y contiene
instrucciones, convenciones y reglas obligatorias para agentes de código que
...
**Nombre**: Geo-Rect - Sistema Híbrido de Validación Geoespacial
**Autor/Contacto**: Arthur Zizumbo
**Tipo**: Proyecto MLOps / Computer Vision + Geospatial
**Stack**: Python 3.11+, Poetry, DVC, MLflow, PyTorch, OpenCV, Kornia, FastAPI, DINOv2
**Objetivo**: Detectar discrepancias geométricas y semánticas entre vectores
de manzanas catastrales y la realidad satelital (5,000 manzanas, 300 etiquetadas)
...
## 🔧 Adaptaciones específicas para Geo-Rect (Reglas del proyecto)

- **Regla obligatoria de US (Historia de Usuario):** Cuando se programe o implemente
    cualquier Historia de Usuario (US), el agente **debe** generar únicamente UN
    archivo Markdown en la ruta `docs/us-resolved/us.<US-ID>.md` (por ejemplo
    `docs/us-resolved/us.1.1.md`) y **ningún otro** archivo de documentación en
    `docs/` relacionado con esa US. Ese archivo será la única fuente canónica
    para la descripción de la implementación, criterios de aceptación, pasos de
    prueba, comandos útiles y enlaces a artefactos (DVC, MLflow, modelos). Si la
    US evoluciona, el agente actualizará ese mismo archivo en lugar de crear
    duplicados.

- **Ubicación de código y artefactos:** El código producido por la US debe
    ir en `src/` según la estructura del proyecto; `docs/us-resolved/` sólo será
    usado para la documentación-resumen de la US. No crear archivos de código en
    `docs/` ni múltiples `us.*` para la misma historia.

- **Nombre de archivo obligatorio:** `docs/us-resolved/us.<US-ID>.md` (usar el
    mismo ID que aparece en la planificación: `US-1.1` → `us.1.1.md`).

## 🧠 Buenas prácticas de Visión por Computador (adaptadas)

Estas recomendaciones son específicas para proyectos geoespaciales y de
visión que operan bajo restricciones de datos y VRAM (ej. RTX 4070, 8GB):

- **Preprocesamiento geoespacial:** siempre mantener CRS y metadatos durante
    rasterización; documentar transformaciones. Usar `rasterio`/`pyproj` para
    reproyecciones y `shapely` para operaciones geométricas.

- **Tile stitching y caching:** descargar tiles de forma asíncrona con retry
    y cache (ej. `diskcache`). Cuando formes mosaicos, normaliza iluminación y
    tamaño de tile; guarda checksum para reproducibilidad.

- **Augmentations geométricos y fotométricos:** aplicar augmentations que
    respeten la topología: rotaciones por múltiplos de 90° cuando corresponda,
    flips controlados, cambios de brillo/contraste y jitter geométrico para
    simular desalineamientos reales.

- **Few-shot / Synthetic data:** con sólo 300 etiquetas, generar augmentations
    sintéticas y ejemplos negativos/positivos balanceados; documentar la receta
    y versionarla con DVC cuando vaya a producción.

- **Embeddings y modelos ligeros:** precomputar embeddings (DINOv2) offline
    y guardarlos como artefactos (DVC). Evitar recomputar en cada request; usar
    batching y FP16 donde sea seguro.

- **Cascada de alineación:** implementar y testear ECC → LoFTR → SAM como
    fallback; cada paso debe exponer una métrica de confianza (por ejemplo
    `cc` para ECC, `#matches` para LoFTR) y un umbral documentado.

- **Métricas de evaluación:** además de F1/AUC, registrar IoU, Hausdorff,
    Dice, diferencia de centroides y correlación de contornos. Loguear todas
    las métricas en MLflow por run.

- **Monitoreo de recursos:** instrumentar uso de VRAM y tiempo por batch;
    establecer alertas localmente (logs) y límites seguros (ej. batch size que
    mantenga VRAM < 7GB). Preferir inferencia por CPU cuando GPU no está
    disponible y documentar degradación.

- **Determinismo y reproducibilidad:** fijar seeds, documentar entornos
    (`pyproject.toml` + `poetry.lock`), y versionar datasets/modelos con DVC.

- **Testing para módulos CV:** agregar tests unitarios e integración para
    `ecc_aligner`, `loftr_aligner`, `sam_validator`, `vector_rasterizer` y
    `tile_stitcher` que ejecuten rutas felices y fallas controladas (mocking
    I/O y recursos pesados).

- **Optimización de inferencia:** exportar modelos ligeros (ONNX/torchscript)
    con perfiles de latencia y memoria; documentar cómo cargar y ejecutar en
    FastAPI (ej. pre-warm, pool de workers, caching de resultados).

## ⚙️ Otras mejores prácticas MLOps aplicables

- **Artefactos:** guardar pesos y datasets versionados con DVC; registrar
    métricas y parámetros en MLflow por experimento/run.

- **CI/CD:** los pipelines deben ejecutar `poetry install --with dev`,
    `ruff check`, `pytest` (con coverage) y pasos de sanity para DVC.

- **Comandos reproducibles:** documentar comandos para reproducir entrenamiento
    y evaluación en `AGENTS.md` y en cada `docs/us-resolved/us.<US-ID>.md` si
    aplica.

- **Seguridad y claves:** nunca commitear API keys; usar `.env` y abstraer
    clientes HTTP para tests (mocks). Documentar limits y costos de APIs
    externas (Google Maps).
